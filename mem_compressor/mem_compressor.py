...
class DynamicLatentMemoryCompressor(nn.Module):
    ...
    def __init__(
        ...
        self.memory_slots = nn.Parameter(
            torch.randn(num_memory_slots, memory_dim)
        )
    ...
    def encode_experience(
        self, 
        llm_hidden_states: torch.Tensor  # [batch, seq_len, llm_hidden_size]
    ) -> torch.Tensor:
        """将LLM的隐藏状态编码为记忆表示"""
        # 压缩到记忆空间
        memory_repr = self.memory_encoder(llm_hidden_states)  # [batch, seq_len, memory_dim]
        return memory_repr
    ...
    def update_memory(
        ...
        if current_memory is None:
            current_memory = self.memory_slots.unsqueeze(0).expand(
                batch_size, -1, -1
            )
        ...
        updated_memory = (
            update_gate * attended_memory + 
            (1 - update_gate) * current_memory
        )
        
        # 计算记忆重要性分数
        importance_scores = self.importance_scorer(updated_memory)  # [batch, num_slots, 1]
    ...
    def retrieve_memory(
        ...
        # 检索记忆
        retrieved_memory = torch.bmm(attn_weights, memory)  # [batch, seq_len, memory_dim]
...
class MemoryAugmentedLLMAgent(nn.Module):
    ...
    def __init__(
        ...
        # 记忆注入层（将检索到的记忆融入LLM）
        self.memory_injection = nn.Linear(
            llm_hidden_size * 2, 
            llm_hidden_size
        )
...
class MemoryTrainer:
    ...
    def compute_memory_loss(
        ...
        if use_importance_regularization and outputs['importance_scores'] is not None:
            ...
            # 熵正则化鼓励记忆分化
            importance_dist = importance_scores / importance_scores.sum(dim=-1, keepdim=True)
            entropy = -(importance_dist * torch.log(importance_dist + 1e-10)).sum(dim=-1).mean()
